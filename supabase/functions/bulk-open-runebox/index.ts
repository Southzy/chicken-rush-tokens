...
import { RuneboxSchema } from "../_shared/validation.ts";
import { pfHash, hexToModInt } from "../_shared/provablyFair.ts";
...
function normalize<T extends { dropRate: number }>(arr: T[]): T[] {
  const s = arr.reduce((a, r) => a + r.dropRate, 0);
...
function poolOf(boxType: BoxType): Rune[] { return boxType === "special" ? SPECIAL : BASE; }
function priceOf(boxType: BoxType): number { return boxType === "special" ? BOX_PRICE_SPECIAL : BOX_PRICE_BASIC; }
...
function applyDiminishing(cur: number, cap: number | null, gain: number) {
  if (!cap) return gain;
  if (cur >= cap) return 0;
  if (cur >= cap * 0.9) return gain * 0.5;
  return gain;
}
...
function rollOnce(inv: Record<string, number>, pool: Rune[], nonce: string): OpenResult {
  ...
  for (const rune of pool) {
    ...
    if (r <= cum) {
      ...
      return { runeKey: rune.key, actualGain: gain, wasCapHit: rune.cap != null ? cur >= rune.cap : false, nonce };
    }
  }
